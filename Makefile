...
# executable
EXE_FILES =	minigfs_client minigfs_master minigfs_primary minigfs_secondary_A minigfs_secondary_B run

# rules

all: 	minigfs_client minigfs_master minigfs_primary minigfs_secondary_A minigfs_secondary_B run
...
run.o:		run.cpp Flight.h Seller.h $(CORE_INCS)
	$(CC) -c $(CFLAGS) run.cpp
...
run:	$(CORE_OBJS) run.o base64.o
	$(CC) -o run $(CORE_OBJS) run.o base64.o $(LDFLAGS_SV) $(LDFLAGS_CL)
